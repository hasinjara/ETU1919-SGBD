...
public class Fonction {
    ...
    public void affTable(TableRelationnelle table) {
        Object[][] data = toTabObjects(table);
        int maxsize = maxSize(table, data);
        int numberOfcol = numberOfcol(table);
        System.out.println(table.getNom());
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            for (int j = 0; j < data[i].length; j++) {
                int maxIncol = maxIncol(data,j);
                String espace = generate(" ",(maxIncol-(data[i][j]).toString().length())+1 );
                String simpleData = " " +data[i][j] + espace;
                if(i == 0) {
                    String[] split = data[i][j].toString().split("\\ ");
                    espace = generate(" ",(maxIncol-(split[1]).toString().length())+1 );
                    simpleData =" "+ split[1]+espace ;
                }
                if(j == 0) {
                    simpleData = "| "+data[i][j] + espace;
                    if(i == 0) {
                        String[] split = data[i][j].toString().split("\\ ");
                        espace = generate(" ",(maxIncol-(split[1]).toString().length())+1 );
                        simpleData = "| "+split[1] + espace;
                ...
                simpleData = makeInCol(simpleData); 
    ...
    String makeInCol(String n) {
        return n+"|";
    }

    String bordrure(TableRelationnelle tab, Object[][] data) {
        int cols = numberOfcol(tab);
        String val = "+";
        for(int i = 0; i<cols; i++){
           int maxpercol = maxIncol(data, i);
           String line = generate("-",maxpercol+2) + "+";
           val = val + line;
        }
    return val;
    }

    Object[] getColumn(Object[][] tab, int col) {
    ...
    int maxIncol(Object[][] tab, int col) {
        Object[] cols = getColumn(tab, col);
        String[] split = cols[0].toString().split("\\ ");
        int max = split[1].toString().length();
        //int max = cols[0].toString().length();
        for(int i = 1; i<cols.length; i++) {
            if(cols[i].toString().length() > max) {
                max = cols[i].toString().length();
            }
        }
    return max; 
    }

    int maxSize(TableRelationnelle tab, Object[][] data) {
        int max = 0;
        int cols = numberOfcol(tab);
        //System.out.println(cols);
        for(int i = 0; i<cols; i++){
            //System.out.println(maxIncol(data,i));
            max = max + maxIncol(data,i);
        }
    return max;
    }

    int numberOfcol(TableRelationnelle table) {
        Object[] lo = table.getDonnee();
        String attr = lo[0].toString();
        String[] all = attr.split("\\,");
    return all.length;
    }

    public Object[][] toTabObjects(TableRelationnelle table) {
        Object[] data = table.getDonnee();
        int line = data.length;
        String[] split_col = data[0].toString().split("\\,");
        int col = split_col.length;
        Object[][] val = new Object[line][col];
        for (int i = 0; i < data.length; i++) {
    ...
    Object[] indSelectOneCond(TableRelationnelle table, String cond) {
        ...
        for (int i = 1; i < tmp.length; i++) {
            ...
            if(operation.compareToIgnoreCase("like") == 0) {
